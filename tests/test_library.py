...
def test_rca38_decode_encode():
    ...
    assert data == list(irgen.gen_raw_general('rca38', 12, 123))


def test_gen_bitstream_from_raw():
    assert list(irgen.gen_bitstream_from_raw([100, 200], 100)) == [1, 1, 1]
    assert list(irgen.gen_bitstream_from_raw([100, -200], 100)) == [1, -1, -1]
...
@pytest.mark.parametrize("protocol, device, function", [
    ("nec1", 1, 4),
    ("necx1", 13330, 1)
])
def test_nec_round_about(protocol, device, function):
    data = []
    data.extend(irgen.gen_raw_nec(protocol, device, function))
    x = iter(data)
    assert irgen.dec_raw_nec(x, protocol=protocol) == (device, function, 0)
